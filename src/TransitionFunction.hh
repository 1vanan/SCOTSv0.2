 ...
 *  ?!? contains all graph? parallel it? Yes
 **/
    class TransitionFunction {
    public:
        ...
        std::unique_ptr<abs_type[]> m_pre;         // 1d array, size T, type inside abs_type. keeps pointers on pre. For one node ?!?
        /** @brief array[N*M] containing the pre's address in the array m_pre[T] **/
        std::unique_ptr<abs_ptr_type[]> m_pre_ptr; // 1d array, size N*M. Keeps pre. Hash function. Shows link on link on memory by state-input
    ...
    public:
        ...
        TransitionFunction &operator=(TransitionFunction &&other) {
            m_no_states = std::move(other.m_no_states); // assign other.m_no_states to m_no_states
        ...
        std::vector <abs_type> get_post(const abs_type &i, const abs_type &j) const {
            ...
            if (m_pre != nullptr) {
                ...
                std::ostringstream os;
                os
                        << "TransitionFunction.hh: Error: Unable to get post. Transition relation is empty, i.e.,  m_pre and post_ are nullptr.";
                throw std::runtime_error(os.str().c_str());
            }
            return post;
        }
